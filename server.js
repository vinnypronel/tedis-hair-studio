...
async function run(query, params = []) {
  const converted = convertSql(query);
  let finalQuery = converted;
  ...
  try {
    ...
    return { id: result[0]?.id || null, changes: result.length, rows: result };
  } catch (e) {
    if (e.message.includes('unique constraint') || e.message.includes('UNIQUE constraint')) {
      return { id: null, changes: 0, error: 'UNIQUE' };
    }
    throw e;
  }
}
...
async function all(query, params = []) {
  return await sql(convertSql(query), params);
}

async function get(query, params = []) {
  const result = await sql(convertSql(query), params);
...
async function autoGenerateAllSlots() {
  ...
  for (let d = new Date(today); d <= endDate; d.setDate(d.getDate() + 1)) {
    ...
    for (let mins = openMins; mins <= lastMins; mins += 30) {
      ...
      try {
        await run(
          `INSERT INTO time_slots (date, time, is_available) VALUES (?, ?, 1) ON CONFLICT (date, time) DO NOTHING`,
          [dateStr, timeStr]
        );
        slotsCreated++;
      } catch (e) {
...
async function migrateBookingsTable() {
  try {
    const columns = await all("SELECT column_name FROM information_schema.columns WHERE table_name = 'bookings'");
...
async function initUsers() {
  try {
    ...
    const oldTable = await get("SELECT tablename FROM pg_catalog.pg_tables WHERE tablename = 'admin_users'");
    if (oldTable) {
      ...
      for (let m = openMin; m <= lastMin; m += interval_minutes) {
        ...
        await run(
          `INSERT INTO time_slots (date, time, is_available)
           VALUES (?, ?, 1) ON CONFLICT (date, time) DO NOTHING`,
          [dateStr, t]
        );
        created++;
      }
